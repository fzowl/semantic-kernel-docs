...
For RAG applications where you need document-aware chunk embeddings, use the contextualized embedding model `voyage-context-4`.

When you enable automatic chunking, pass a flat `list[str]` of full-document strings as the input. The service chunks each document server-side, so you don't need to pre-chunk the text yourself. Auto-chunking requires the `document` input type, and `chunk_size` accepts a target of up to 32,000 tokens per chunk.
...
embedding_service = VoyageAIContextualizedEmbedding(
    ai_model_id="voyage-context-4",
    api_key=os.getenv("VOYAGE_API_KEY"),
)

# With auto-chunking, inputs is a flat list[str] of full-document strings
inputs = [
    "Chapter 1: Introduction. This chapter covers the key concepts...",
    "Chapter 2: Methods. We employed the following approach...",
]

# Calls the underlying contextualized_embed API with a list[str] input,
# auto-chunking enabled, and a 32,000-token chunk size.
embeddings = await embedding_service.generate_contextualized_embeddings(
    inputs,
    input_type="document",
...
VoyageAITextEmbeddingGenerationService textEmbeddingService =
    VoyageAITextEmbeddingGenerationService.builder()
        .withClient(client)
        .withModelId("voyage-4-large")  // Options: voyage-4-large, voyage-code-4, voyage-finance-2, voyage-law-2
        .build();
```
