...
# [VoyageAI by MongoDB](#tab/csharp-VoyageAI)
...
> [!IMPORTANT]
> The VoyageAI by MongoDB embedding generation connector is currently experimental. To use it, you will need to add `#pragma warning disable SKEXP0070`.
...
kernelBuilder.AddVoyageAITextEmbeddingGeneration(
    modelId: "voyage-4-large",           // Name of the VoyageAI by MongoDB model, e.g. "voyage-4-large", "voyage-code-4", "voyage-finance-2"
    apiKey: "YOUR_API_KEY",
    endpoint: "https://api.voyageai.com/v1", // Optional; VoyageAI by MongoDB API endpoint
    serviceId: "YOUR_SERVICE_ID",        // Optional; for targeting specific services within Semantic Kernel
    httpClient: new HttpClient()         // Optional; if not provided, the HttpClient from the kernel will be used
);
Kernel kernel = kernelBuilder.Build();
...
builder.Services.AddVoyageAITextEmbeddingGeneration(
    modelId: "voyage-4-large",                  // Name of the VoyageAI by MongoDB model
    apiKey: "YOUR_API_KEY",
    endpoint: "https://api.voyageai.com/v1",    // Optional; VoyageAI by MongoDB API endpoint
    serviceId: "SERVICE_ID"                     // Optional; for targeting specific services within Semantic Kernel
);
...
# [VoyageAI by MongoDB](#tab/python-VoyageAI)
...
text_embedding_service = VoyageAITextEmbedding(
    ai_model_id="voyage-4-large",  # Options: voyage-4-large, voyage-4, voyage-4-lite, voyage-code-4, voyage-finance-2, voyage-law-2
...
## VoyageAI by MongoDB Specialized Services

VoyageAI by MongoDB provides additional specialized embedding services for advanced use cases.
...
embedding_service = VoyageAIContextualizedEmbedding(
    ai_model_id="voyage-context-4",  # Options: voyage-context-4, voyage-context-3
    api_key=os.getenv("VOYAGE_API_KEY"),
)

# The underlying contextualized_embed API accepts inputs typed as
# Union[List[List[str]], List[str]] - both formats are supported.

# Format 1 - List[List[str]]: each inner list holds the chunks of a single document.
inputs = [
...
# Format 2 - List[str]: a flat list of chunks that share a single context.
flat_inputs = ["Introduction to the topic", "Supporting details", "Concluding remarks"]
embeddings = await embedding_service.generate_contextualized_embeddings(flat_inputs)
```

For the full parameter list, see the official [contextualized chunk embeddings documentation](https://docs.voyageai.com/docs/contextualized-chunk-embeddings).
...
embedding_service = VoyageAIMultimodalEmbedding(
    ai_model_id="voyage-multimodal-3.5",  # Options: voyage-multimodal-3.5, voyage-multimodal-3
    api_key=os.getenv("VOYAGE_API_KEY"),
)
...
VoyageAI by MongoDB also provides document reranking for improved search results:
...
reranker_service = VoyageAIReranker(
    ai_model_id="rerank-2.5",  # Options: rerank-3, rerank-3-lite, rerank-2.5, rerank-2.5-lite
    api_key=os.getenv("VOYAGE_API_KEY"),
)
...
# [VoyageAI by MongoDB](#tab/java-VoyageAI)
...
// Create VoyageAI by MongoDB client
...
VoyageAITextEmbeddingGenerationService textEmbeddingService =
    VoyageAITextEmbeddingGenerationService.builder()
        .withClient(client)
        .withModelId("voyage-4-large")  // Options: voyage-4-large, voyage-4, voyage-4-lite, voyage-code-4, voyage-finance-2, voyage-law-2
        .build();
```
...
VoyageAIContextualizedEmbeddingGenerationService service =
    VoyageAIContextualizedEmbeddingGenerationService.builder()
        .withClient(client)
        .withModelId("voyage-context-4")  // Options: voyage-context-4, voyage-context-3
        .build();

// The underlying contextualized_embed API accepts inputs typed as
// Union[List[List[str]], List[str]] - both formats are supported.

// Format 1 - List<List<String>>: each inner list holds the chunks of a single document.
...
// Format 2 - List<String>: a flat list of chunks that share a single context.
List<String> flatInputs = Arrays.asList("Introduction to the topic", "Supporting details", "Concluding remarks");
List<Embedding> flatEmbeddings = service.generateContextualizedEmbeddingsAsync(flatInputs).block();
```
...
VoyageAIMultimodalEmbeddingGenerationService service =
    VoyageAIMultimodalEmbeddingGenerationService.builder()
        .withClient(client)
        .withModelId("voyage-multimodal-3.5")  // Options: voyage-multimodal-3.5, voyage-multimodal-3
        .build();
